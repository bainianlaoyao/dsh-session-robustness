 ...
 * (`MALFORMED_RESPONSE`), unclassified stream (`STREAM`), and HTTP 408 /
 * 409 / 425 / 429 / 499 / 5xx.
 *
 ...
 * Catch-all codes (`PI_AI_ERROR` / `UNKNOWN`) still use a message heuristic.
 * Permanent failures (AUTH / QUOTA / abort / missing or invalid
 * credentials / context overflow / no adapter) are never retried.
 *
 * Does not wrap `ctx.llm.stream()`: a raw stream cannot separate already
 * emitted chunks durably. Recovery stays on the open-step loop boundary
 * so each retry re-runs the same step over the same durable history.
 */
...
function isNetworkCode(code) {
  ...
  return isNetworkHttpStatus(httpStatusOf(code, null));
}
...
function isRetryableFailure(config, failure) {
  ...
  if (isRetryable(config, code)) return true;
  if (!CATCHALL_CODES.includes(code)) return false;
  return isTransientMessage(failure && failure.message);
}
